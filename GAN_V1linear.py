...
class GeneratorNet(nn.Module):
    def __init__(self,input_size):#解码器
        super().__init__()
        gen=nn.Sequential()
...
#train_loader是一个维度为4的张量，张量分别是（batch_size,map_channels,x y）四个维度。
...
for epoch in range(num_epoches):
    ...
    for i ,(img,_)in enumerate(train_loader):#利用enumerate取出一个可迭代对象的内容
        num_img=img.size(0) # 获取图片的大小，获取行数 batch_size的大小num_img
#        print(num_img)
#        input('休眠N秒')
        img=img.view(num_img,-1)  # img.view与img.reshape功能类似，变成N*1的张量
        if torch.cuda.is_available():
    ...
    if epoch==0:
        ...
        save_image(real_images,'./imgv1/real_iamges.png')
        
    fake_images=to_img(fake_img.cpu().data)
    save_image(fake_images,'./imgv1/fake_image-{}.png'.format(epoch+1))

torch.save(G.state_dict(),'./imgv1/generator.pth')   #保存模型
torch.save(D.state_dict(),'./imgv1/discriminator.pth')    # 保存模型    
